...
static const wchar_t* kInsetFaceIconFileName = L"cmd_insetface.svg";
static const unsigned int kInsetFaceInstallAttemptVersion = 2;
static const unsigned int kInsetFaceIconInstallVersion = 2;
static const unsigned int kInsetFaceLayoutPatchVersion = 2;
...
struct InstallDiagnostics
{
	...
	unsigned int WhiteResourceSize;
	unsigned int BlackResourceSize;
	unsigned int LayoutFileExists;
	unsigned int LayoutWidgetExists;
	unsigned int LayoutChanged;
	std::wstring Result;

	InstallDiagnostics()
		: AttemptVersion(kInsetFaceInstallAttemptVersion)
		, LastError(0)
		, WhiteResourceSize(0)
		, BlackResourceSize(0)
		, LayoutFileExists(0)
		, LayoutWidgetExists(0)
		, LayoutChanged(0)
		, Result(L"not_started")
	{
	}
};

static void SaveInstallDiagnostics(MQSetting* setting, const InstallDiagnostics& diagnostics)
{
	if (setting == NULL) return;

	setting->Save("InstallAttemptVersion", diagnostics.AttemptVersion);
	setting->Save("InstallStage", diagnostics.Stage);
	setting->Save("InstallLastError", diagnostics.LastError);
	setting->Save("InstallDataPath", diagnostics.DataPath);
	setting->Save("InstallWhiteIconPath", diagnostics.WhiteIconPath);
	setting->Save("InstallBlackIconPath", diagnostics.BlackIconPath);
	setting->Save("InstallLayoutPath", diagnostics.LayoutPath);
	setting->Save("InstallTempPath", diagnostics.TempPath);
	setting->Save("InstallWhiteResourceSize", diagnostics.WhiteResourceSize);
	setting->Save("InstallBlackResourceSize", diagnostics.BlackResourceSize);
	setting->Save("InstallLayoutFileExists", diagnostics.LayoutFileExists);
	setting->Save("InstallLayoutWidgetExists", diagnostics.LayoutWidgetExists);
	setting->Save("InstallLayoutChanged", diagnostics.LayoutChanged);
	setting->Save("InstallResult", diagnostics.Result);
}

static void SetInstallStage(MQSetting* setting, InstallDiagnostics& diagnostics, const wchar_t* stage)
{
	diagnostics.Stage = stage != NULL ? stage : L"";
	diagnostics.LastError = 0;
	diagnostics.Result = L"running";
	SaveInstallDiagnostics(setting, diagnostics);
}

static void FailInstallStage(MQSetting* setting, InstallDiagnostics& diagnostics, const wchar_t* stage, DWORD error_code)
{
	diagnostics.Stage = stage != NULL ? stage : L"";
	diagnostics.LastError = static_cast<unsigned int>(error_code);
	diagnostics.Result = L"failed";
	SaveInstallDiagnostics(setting, diagnostics);
}

static std::wstring BuildInstallFailureStatus(const InstallDiagnostics& diagnostics)
{
	if (diagnostics.Stage.empty()) {
		return L"Inset Face icon install failed";
	}

	wchar_t buffer[256];
	swprintf_s(buffer, L"Inset Face icon install failed: %s (err=%u)", diagnostics.Stage.c_str(), diagnostics.LastError);
	return buffer;
}
...
static bool ReadBinaryFile(const std::wstring& path, std::string& out_data)
{
	...
	BOOL ok = TRUE;
	if (file_size > 0) {
		ok = ReadFile(file, &data[0], file_size, &bytes_read, NULL);
	}
	CloseHandle(file);
...
static bool WriteBinaryFileAtomic(const std::wstring& path, const std::string& data)
{
	...
	if (!EnsureDirectoryExists(directory)) return false;
	...
	BOOL ok = TRUE;
	if (!data.empty()) {
		ok = WriteFile(file, data.data(), (DWORD)data.size(), &bytes_written, NULL);
	}
	if (ok) {
...
static bool LoadResourceBytes(int resource_id, std::string& out_data)
{
	HRSRC resource = FindResourceW(g_hInstance, MAKEINTRESOURCEW(resource_id), reinterpret_cast<LPCWSTR>(RT_RCDATA));
	if (resource == NULL) return false;

	DWORD resource_size = SizeofResource(g_hInstance, resource);
	if (resource_size == 0) return false;

	HGLOBAL handle = LoadResource(g_hInstance, resource);
...
static std::wstring BuildPluginWidgetName()
{
	wchar_t buffer[64];
	swprintf_s(buffer, L"BtnComPlugin.%08x.%08x.0", kInsetFaceProductId, kInsetFacePluginId);
	return buffer;
}

static std::wstring BuildIconFileName()
{
	return kInsetFaceIconFileName;
}
...
{
	std::wstring filename = GetFileNameOnly(path);
	for (size_t i = 0; i < filename.size(); ++i) {
		if (filename[i] >= L'A' && filename[i] <= L'Z') {
			filename[i] = filename[i] - L'A' + L'a';
		}
	}
...
static bool ParseWidgetTag(const std::string& tag_text, XmlWidgetTag& out_tag)
{
	...
	while (pos < tag_text.size()) {
		...
		if (pos >= tag_text.size()) break;
		if (tag_text[pos] == '/' || tag_text[pos] == '>') break;
		...
		size_t value_end = tag_text.find('"', value_start);
		if (value_end == std::string::npos) return false;

		std::string value = tag_text.substr(value_start, value_end - value_start);
		pos = value_end + 1;
...
static void ConfigureWidgetAttributes(LayoutPatchKind kind, XmlWidgetTag& tag, bool existing_widget)
{
	const std::string icon_name = NarrowAscii(BuildIconFileName());

	switch (kind) {
	case LayoutPatchKind::Default:
		SetWidgetAttribute(tag, "parent", "FrameComMiscButtons", !existing_widget ? true : false);
		break;
	case LayoutPatchKind::StandardIcon:
		...
		SetWidgetAttribute(tag, "icon", icon_name, true);
...
static bool PatchLayoutText(std::string& text, LayoutPatchKind kind, const std::string& widget_name)
{
	if (kind == LayoutPatchKind::Unknown) return false;

	const std::string marker = "name=\"" + widget_name + "\"";
	size_t name_pos = text.find(marker);
	if (name_pos != std::string::npos) {
		size_t tag_start = text.rfind("<widget", name_pos);
		size_t tag_end = text.find("/>", name_pos);
		if (tag_start == std::string::npos || tag_end == std::string::npos || tag_end < tag_start) {
			return false;
		}

		std::string old_tag = text.substr(tag_start, tag_end + 2 - tag_start);
		XmlWidgetTag tag;
		if (!ParseWidgetTag(old_tag, tag)) return false;
		ConfigureWidgetAttributes(kind, tag, true);
	...
	new_tag.Attributes["name"] = widget_name;
	ConfigureWidgetAttributes(kind, new_tag, false);

	std::string insert_tag = BuildWidgetTagString(new_tag);
	if (kind != LayoutPatchKind::Default) {
		insert_tag += "<!--InsetFace-->";
	}
...
private:
	...
	bool EnsureCommandIconInstalled();
	bool EnsureSvgFile(const std::wstring& path, int resource_id);
	bool EnsureLayoutPatched(const std::wstring& layout_path);
...
bool InsetFacePlugin::EnsureSvgFile(const std::wstring& path, int resource_id)
{
	std::string resource_data;
	if (!LoadResourceBytes(resource_id, resource_data)) {
		return false;
	}

	std::string current_data;
	if (ReadBinaryFile(path, current_data) && current_data == resource_data) {
		return true;
	}

	return WriteBinaryFileAtomic(path, resource_data);
}

bool InsetFacePlugin::EnsureLayoutPatched(const std::wstring& layout_path)
{
	DWORD attributes = GetFileAttributesW(layout_path.c_str());
	if (attributes == INVALID_FILE_ATTRIBUTES) {
	...
	if (!PatchLayoutText(patched_text, kind, NarrowAscii(BuildPluginWidgetName()))) {
		return false;
	}
	...
	return WriteBinaryFileAtomic(layout_path, patched_text);
}

bool InsetFacePlugin::EnsureCommandIconInstalled()
{
	wchar_t data_path[MAX_PATH];
	if (MQ_GetSystemPathW == NULL || !MQ_GetSystemPathW(data_path, MQFOLDER_DATA)) {
		return false;
	}

	unsigned int installed_icon_version = 0;
	unsigned int installed_layout_version = 0;
	MQSetting* setting = OpenSetting();
	if (setting != NULL) {
		setting->Load("IconInstallVersion", installed_icon_version, 0U);
		setting->Load("LayoutPatchVersion", installed_layout_version, 0U);
	}

	std::wstring icon_root = JoinPath(data_path, L"Icon");
	std::wstring layout_root = JoinPath(data_path, L"Layout");
	std::wstring icon_name = BuildIconFileName();

	bool ok = true;
	ok = EnsureSvgFile(JoinPath(JoinPath(icon_root, L"White"), icon_name), IDR_ICON_WHITE_SVG) && ok;
	ok = EnsureSvgFile(JoinPath(JoinPath(icon_root, L"Black"), icon_name), IDR_ICON_BLACK_SVG) && ok;
	ok = EnsureLayoutPatched(JoinPath(layout_root, L"default.xml")) && ok;
	ok = EnsureLayoutPatched(JoinPath(layout_root, L"standard_icon.xml")) && ok;
	ok = EnsureLayoutPatched(JoinPath(layout_root, L"basic.xml")) && ok;
	ok = EnsureLayoutPatched(JoinPath(layout_root, L"mapping.xml")) && ok;

	if (setting != NULL) {
		if (ok) {
			setting->Save("IconInstallVersion", kInsetFaceIconInstallVersion);
			setting->Save("LayoutPatchVersion", kInsetFaceLayoutPatchVersion);
		}
		CloseSetting(setting);
	}

	(void)installed_icon_version;
	(void)installed_layout_version;
	return ok;
}
